...
export function useBridge() {
  ...
  /** completed | cancelled — fully done items in a plan card */
  ...
  const applyEvent = useCallback((event: DomainEvent) => {
    ...
    switch (event.type) {
      ...
      case "UserMessageAppended": {
        ...
        setTasks((ts) =>
          ts.length > 0 && ts.every(isTaskSettled) ? [] : ts
        );
        assistantBuf.current = "";
        thoughtBufMap.current.clear();
        postToolsAssistantId.current = null;
      ...
      case "TasksReplaced": {
        // Empty plan = Core dismissed it. All-done list stays until next user turn.
